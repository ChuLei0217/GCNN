...
def toy_dataset(num_nodes, num_node_features, num_edges):
    ...
    edge_index = np.random.randint(low=0, high=num_nodes - 1, size=[2, num_edges], dtype=np.long)

    data = Data(x=torch.from_numpy(x), dge_index=torch.from_numpy(edge_index))
...
if __name__ == '__main__':
    ...
    # print(toy_sample)
    toy_data = PyGToyDataset(save_root="toy")
    print(toy_data[0])
